        ...
        try:
            ...
            if latest_issue_id != session_state["last_known_issue_id"]:
                ...
                if last_bet and str(last_bet["issue_id"]) == latest_issue_id:
                    ...
                    # Lưu ý: Nếu cược lần 2 thất bại, ta vẫn gửi 0.1 (chỉ là nó không thành công)
                    # Tuy nhiên, nếu cược lần 2 *thành công*, `final_bet_amount_from_last_round` sẽ là 0.1.
                    # Nếu cược lần 2 *thất bại*, `final_bet_amount_from_last_round` sẽ là `initial_bet_amount_from_last_round` (đã sửa ở cuối vòng trước),
                    # Tức là tiền cược lần 2 không được tính vào tổng.
                    # Dùng 0.1 để tính cho trường hợp cược lần 2 thành công là hợp lý hơn cho tổng quan:
                    # Sửa lại: total_bet_for_summary = initial_bet_amount_from_last_round + (0.1 if final_bet_amount_from_last_round == 0.1 else 0)
                    # Sửa lại theo code cũ: total_bet_for_summary = initial_bet_amount_from_last_round + final_bet_amount_from_last_round
                    # Tuy nhiên nếu cược 2 thất bại, `final_bet_amount_from_last_round` lại là `initial_bet_amount_from_last_round` (BUG LOGIC CŨ)
                    # Cập nhật lại: Tính tổng tiền cược dựa trên ý định:
                    if final_bet_amount_from_last_round == 0.1:
                        total_bet_for_summary = initial_bet_amount_from_last_round + 0.1
                    else:
                        # Cược lần 2 thất bại (hoặc không xảy ra), chỉ tính cược lần 1
                    ...
                    if is_win:
                        ...
                        # Tính Lợi nhuận chính xác: (Số dư cuối - Số dư đầu)
                        round_profit = (final_balance - balance_before) if final_balance is not None and balance_before is not None else 0
                        # Lợi nhuận lý thuyết: (Tổng tiền thắng * 0.95) - Tổng tiền cược
                        # Nếu cược 2 thành công: (initial_bet * 0.95) + (0.1 * 0.95) - (initial_bet + 0.1)
                        # Nếu cược 2 thất bại: (initial_bet * 0.95) - initial_bet
                        
                        session_state["rounds_to_skip_after_loss"] = 0 # NEW: Reset nghỉ sau thua nếu thắng
                    else: # THUA
                        ...
                        # Lỗ chính xác: (Số dư cuối - Số dư đầu) hoặc là Tổng tiền cược (nếu không lấy được số dư)
                        ...
                        # NEW: Đặt ván nghỉ ngẫu nhiên từ 3 đến 5 sau khi thua
                        session_state["rounds_to_skip_after_loss"] = random.randint(3, 5)
                        console.print(Panel(f"❌ THUA VÁN! Kích hoạt cơ chế nghỉ [bold yellow]{session_state['rounds_to_skip_after_loss']}[/bold yellow] ván để tránh bị game nghi ngờ.", title="[bold red]CƠ CHẾ NGHỈ TRÁNH GAME[/bold red]", border_style="red"))
                    ...
                    # Lấy thông tin cược cuối để hiển thị
                    if final_bet_amount_from_last_round == 0.1:
                        action_text = Text(f"Cược cuối: Phòng {bet_room} ({ROOM_NAMES.get(str(bet_room), '?')})", style=STYLE_INFO)
                    else:
                        action_text = Text(f"Chỉ cược lần 1: Phòng {bet_room} ({ROOM_NAMES.get(str(bet_room), '?')})", style=STYLE_WARNING)
                ...
                if 0 < MAX_LOSE_STREAK <= session_state['lose_streak']:
                    ...
                    if choice in ['n', 'nghi']: console.print("[yellow]Bot đã dừng theo yêu cầu của người dùng.[/yellow]"); return
                        ...
                        session_state["current_bet_amount"] = BASE_BET
                        session_state["rounds_to_skip_after_loss"] = 0 # Reset cả nghỉ sau thua
                ...
                # --- KIỂM TRA CƠ CHẾ NGHỈ SAU KHI THUA (Ưu tiên) ---
                if session_state["rounds_to_skip_after_loss"] > 0:
                    console.print(f"😴 [red]Đang trong thời gian nghỉ sau khi thua. Còn lại [bold]{session_state['rounds_to_skip_after_loss']}[/bold] ván nghỉ...[/red]")
                ...
                if change_bet_response and change_bet_response.get("code") == 0:
                    ...
                    # Cần đảm bảo `session_state["last_bet_on"]` vẫn giữ giá trị của cược lần 1
                    # Cần sửa: nếu cược 2 thất bại, `amount` phải là `bet_amount` (cược lần 1)
                    # Sửa lại: Dù thất bại, ta vẫn ghi nhận cược cuối cùng là cược lần 1.
                    # Bằng cách không sửa gì cả vì nó đã được gán ở trên:
                    # session_state["last_bet_on"] = {"issue_id": next_round_id, "room": initial_room, "amount": bet_amount}
                    # Ta chỉ cần in ra thông báo chính xác.
                    # Ghi nhận cược cuối cùng là cược lần 1 nếu cược lần 2 thất bại:
                    # session_state["last_bet_on"]["room"] = initial_room # Đã là initial_room
                    # session_state["last_bet_on"]["amount"] = bet_amount # Đã là bet_amount
                    
                    console.print(f"❌ [bold red]Cược cuối cùng thất bại! Cược cuối được ghi nhận là cược lần 1: Phòng {initial_room}, Số tiền: {bet_amount} {BET_TYPE}. Phản hồi: {change_bet_response}[/bold red]")
